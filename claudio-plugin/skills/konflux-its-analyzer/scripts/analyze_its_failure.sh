        ...
        while [ $i -lt "$failed_count" ]; do
            ...
            for step_name in $failed_steps; do
                ...
                if [ -n "$log_output" ] && [ "$log_output" != "unauthorized" ]; then
                    step_analysis=$(extract_error_summary "$log_output")
                else
...
extract_error_summary() {
    ...
    local error_lines
    error_lines=$(echo "$logs" | \
        sed 's/^[[:space:]]*//' | \
        grep -i -E 'error|fatal|fail|unauthorized|refused|timeout|killed|denied|crash|panic|oom' | \
        grep -v -i -E '^(time=|level=|msg=)' | \
        tail -5) || true
